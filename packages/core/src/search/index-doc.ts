/**
 * The offline search indexer, invoked inside a tenant transaction. Reads one
 * canonical `fhir_resources` row (RLS-scoped — an invisible resource is a typed
 * error, never an invented row), extracts searchable text, embeds it with the
 * self-hosted provider, and re-materializes the `search_doc` sidecar row
 ...
 * (freshness = projection as-of). `search_doc` survives projections:rebuild, so
 * this is a separate write path from the vd_* upsert.
 */
import { z } from "zod";
