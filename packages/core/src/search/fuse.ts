 ...
 * (search_doc is FORCE-RLS), so this never writes `where practice_id`.
 *
 ...
 * dropped entirely for a zero-norm query; every ORDER BY carries the resource_id
 * tiebreak and is qualified to a real column (never a text output alias).
 */
import type { Fragment } from "postgres";
import { z } from "zod";
import type { TenantSql } from "../db/tenant.js";
import { DEV_MODEL_ID } from "./dev-embedder.js";
...
function lexicalCte(sql: TenantSql, query: string, allowed: readonly string[]): Fragment {
...
function semanticCte(sql: TenantSql, qvec: string | null, allowed: readonly string[]): Fragment {
  ...
  return sql`, semantic as (
    ...
    from (
      ...
      from search_doc
      where search_doc.resource_type = any(${[...allowed]}::text[])
        and search_doc.model_id = ${DEV_MODEL_ID}
...
export async function fuseSearch(
  ...
  const rows = await sql`
    with ${lexicalCte(sql, params.query, params.allowed)}${semanticCte(sql, params.qvec, params.allowed)},
    fused as (
    ...
    from fused
    join search_doc sd
      on sd.resource_id = fused.resource_id and sd.model_id = ${DEV_MODEL_ID}
